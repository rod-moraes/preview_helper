...
/// Picker HSL (igual ao Widgetbook).
...
class _HslColorPickerState extends State<HslColorPicker> {
  ...
  void initState() {
    super.initState();
    final hsl = HSLColor.fromColor(widget.value.toColor());
    hue = hsl.hue.toInt();
    saturation = (hsl.saturation * 100).toInt();
    lightness = (hsl.lightness * 100).toInt();
  }

  void onValueChanged(int newHue, int newSaturation, int newLightness) {
    setState(() {
      hue = newHue;
    ...
    widget.onChanged(OpaqueColor.fromColor(newColor));
  }
      ...
      children: [
        Expanded(
          child: NumberTextField(
            ...
            onChanged: (v) => onValueChanged(v, saturation, lightness),
          ),
        ),
        ...
        Expanded(
          child: NumberTextField.percentage(
            labelText: 'S',
            value: saturation,
            onChanged: (v) => onValueChanged(hue, v, lightness),
          ),
        ),
        const SizedBox(width: 8),
        Expanded(
          child: NumberTextField.percentage(
            labelText: 'L',
            value: lightness,
            onChanged: (v) => onValueChanged(hue, saturation, v),
          ),
        ),
      ],
    );
